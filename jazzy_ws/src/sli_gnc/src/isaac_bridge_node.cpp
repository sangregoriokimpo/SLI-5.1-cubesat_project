...
class IsaacBridgeNode : public rclcpp::Node{
    public:
        IsaacBridgeNode() : Node("isaac_bridge_node"){
            ...
            cli_=create_client<simulation_interfaces::srv::SetEntityState>("/set_entity_state");
            sub_ = create_subscription<orbit_interfaces::msg::OrbitState>(state_topic_, 10,
            [this](orbit_interfaces::msg::OrbitState::SharedPtr msg){
                on_orbit_state(msg);
            });

            check_timer_ = create_wall_timer(std::chrono::seconds(2),
            [this](){
                if(cli_->service_is_ready()){
                    if(!service_ready_){
                        RCLCPP_INFO(get_logger(),"/set_entity_state ready for %s",prim_path_.c_str());
                        service_ready_ = true;
                        check_timer_->cancel();
                    }
                }else{
                    RCLCPP_WARN(get_logger(),"/set_entity_state not ready, is Isaac Sim running?");
                }
            });

            
            RCLCPP_INFO(get_logger(), "isaac_bridge_node: %s -> %s  scale=%.1f",state_topic_.c_str(), prim_path_.c_str(), scale_);
        }
    ...
    private:
        ...
        void on_orbit_state(orbit_interfaces::msg::OrbitState::SharedPtr msg){
            ...
            auto req = std::make_shared<simulation_interfaces::srv::SetEntityState::Request>();
            ...
            cli_->async_send_request(req,[this](rclcpp::Client<simulation_interfaces::srv::SetEntityState>::SharedFuture future){
                try{
                    auto resp = future.get();
                    if(resp->result.result != 1){
                        RCLCPP_WARN(get_logger(),"/set_entity_state failed for %s: %s",prim_path_.c_str(),resp->result.error_message.c_str());
                    }
                }catch (const std::exception& e){
                    RCLCPP_ERROR(get_logger(),"/set_entity_state exception: %s",e.what());
                }
            });

...
using Q4 = std::array<double, 4>;   // (w, x, y, z)
...
static void rot_z(double a, V3& v) {
...
static void rot_x(double a, V3& v) {
    double c = std::cos(a), s = std::sin(a);
...
static std::pair<V3,V3> circular_ic(double mu, double radius,
                                     const std::string& plane) {
    double spd = std::sqrt(mu / radius);
    if (plane == "xy") return {{radius,0,0}, {0,spd,0}};
    if (plane == "xz") return {{radius,0,0}, {0,0,spd}};
    if (plane == "yz") return {{0,radius,0}, {0,0,spd}};
    throw std::invalid_argument("Unknown plane: " + plane);
}

static std::pair<V3,V3> elements_ic(double mu, double a, double e,
                                     double inc_deg, double raan_deg,
                                     double argp_deg, double nu_deg) {
    ...
    // PQW -> world: Rz(raan) * Rx(inc) * Rz(argp)
    auto transform = [&](V3 vec) -> V3 {
        rot_z(argp, vec);
        rot_x(inc,  vec);
        rot_z(raan, vec);
        return vec;
    };
    return {transform(r_pqw), transform(v_pqw)};
}
...
public:
    OrbitVelNode() : Node("orbit_vel_node") {
        ...
        declare_parameter("attractor",    "/World/Earth");
        declare_parameter("orbit_type",   "circular");
        declare_parameter("radius",       6778.0);
        declare_parameter("plane",        "xy");
        declare_parameter("a",            6778.0);
        ...
        if (orbit_type == "circular") {
            auto [r, v] = circular_ic(mu_,
                get_parameter("radius").as_double(),
                get_parameter("plane").as_string());
            r_ = r; v_ = v;
        } else if (orbit_type == "elements") {
            auto [r, v] = elements_ic(mu_,
                get_parameter("a").as_double(),
                get_parameter("e").as_double(),
                get_parameter("inc").as_double(),
                get_parameter("raan").as_double(),
                get_parameter("argp").as_double(),
                get_parameter("nu").as_double());
            r_ = r; v_ = v;
        } else {
        ...
        RCLCPP_INFO(get_logger(),
            "orbit_vel_node: %s  type=%s  mu=%.4f  dt=%.5fs  pub=%.1fHz",
            prim_path_.c_str(), orbit_type.c_str(),
            mu_, dt_sim_, publish_rate);
    }
